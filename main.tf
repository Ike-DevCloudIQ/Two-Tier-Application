...
module "rds" {
  ...
  db-sg-id             = module.security-group.database_security_group_id
...
module "alb" {
  ...
  alb-sg-id           = module.security-group.alb_security_group_id
...
module "autoscaling" {
  ...
  instance-profile-name = var.INSTANCE-PROFILE-NAME
  web-sg-id             = module.security-group.web_security_group_id
  tg-name               = var.TG-NAME
  iam-role              = var.IAM-ROLE
  public-subnet-name1   = var.PUBLIC-SUBNET1
  public-subnet-name2   = var.PUBLIC-SUBNET2
